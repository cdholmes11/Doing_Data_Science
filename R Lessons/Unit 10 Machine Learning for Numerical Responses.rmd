...
dfMPG <- data.frame(MPG = c(42, 40, 38, 28, 32, 30, 18, 20, 22),
    Weight = c(1, 1, 1, 2, 2, 2, 3, 3, 3))

dfMPG %>% ggplot(aes(y = MPG, x = Weight)) +
    geom_point()

# set the y intercept and slope so we can change later to see the effects on
# the SSE
yint <- 50
slope <- -10
...
```{r}
### now lets simulate more samples from distributions described by beta_0 = 50
### and beta_1 = -10 and sigma = 2
...
for (i in 1:numSampIntercepts)
{
    ...
    # make a data frame so we can use lm() and ggplot()
    dffullSample <- data.frame(Weight = weights,
        MPG = c(simWeight1, simWeight2, simWeight3))
...
hist(sampInterceptHolder, main = str_c("Distribution of Intercpets from ",
    numSampIntercepts, " Samples"), xlab = "Sample Intercpet")
...
mtcars %>% ggplot(aes(x = wt, y = mpg)) +
    geom_point()
mtcars2 <- mtcars %>% mutate(wt2 = wt^2)
fit <- lm(mpg ~ wt + wt2, mtcars2)
summary(fit)
...
# vary the last argument  and make last argument in sample call
# below 1 for independent observations.
for (i in 1:6) {
   x <- rep(sample(1:10, 1), 100) # how many we are pulling for each xy pair.
    ...
    z <- 4 + 3 * x - 7 * y + rnorm(100, 0, 7) # make sure the you generate the
                                              # same number of errors as the
                                              # last argument in sample call.
...
set.seed(4)
TrainObs <- sample(seq(1, dim(NBA)[1]), round(.75 * dim(NBA)[1]),
    replace = FALSE)
