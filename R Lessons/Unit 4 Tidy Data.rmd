...
full_data <- merge(demographic, experimentData, c("subject", "condition"))
full_data
```
...
api_key <- "ZOFe4oYBVcylcghj4EsSIHzKm"
api_secret <- "pqaanBlLNQ0mIFMEuVwMBCONTm101t4s44qWH49dmodDpkhTRW"
access_token <- "109746024-y0dECu5zdkL5K88apApg8BYhwpUpPKOuEQPpu0OP"
access_token_secret <- "FPvGHjpaV9CWcsO09p7RP8AfsA4LlP5sW0siXMJillNoe"

# Load twitteR
token <- create_token(
  app = "MSDS Session 4 twitteR package",
...
# Get tweets
tweets <- search_tweets("Texas A&M + Football", n = 10, lang = "en")
...
get_trends(395269) # Caracas, Venezuela

get_trends(2487889) # San Diego, California

get_trends(44418) #  London, England

get_trends(2388929) #  Dallas, US

DallasTrends = get_trends(2388929) %>% select(trend) #  Dallas, US
...
#Show access through URL: https://api-v2.intrinio.com/companies/AAPL?api_key=OjcyZWM5Mzg2NDU1ZWMxYjExYzM1OGEyZTY3NDM1MDNh
...
resultsGoodIndicator <- as.data.frame(resultsGoodIndicator) %>%
    separate(indicator, c("topic", "general", "specific", "extension"))
