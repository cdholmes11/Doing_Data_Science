...
# Probability of positive if testing positive
P(D|+) = P(+|D) P(D) / P(+)
# Probability of positive if testing negative
P(D|-) = P(-|D) P(D) / P(-)
...
# Example 1
For those who have the disease, 90% test positive.
    P(+|D) = .9
    P(-|D) = .1
For those who do not have the disease, 80% test negative.
    P(-|ND) = .8
    P(+|ND) = .2
...
df <- data.frame(Outlook = "Sunny", Wind = "Hi")
...
NYTIMES_KEY = "FDHOzGhKDUs0ArXViMvSfjMyYe4tbJkm" # Your Key Here … get from NYT API website
...
confusionMatrix(as.factor(allNYTSearchTest$Classified),as.factor(allNYTSearchTest$NewsOrOther))
