...
str_view_all(string5,"shot.")
str_view_all(string5,"shot\\.") # Looking for "shot."
...
str_view(string1, "^Don't") # Look at the beginning of the string
str_view(string1, "^interfere") 
str_view(string3, "^T")
str_view(string6, "^\"Some") # There's actually a space before the quote.
str_view(string6, "Jobs$") # Looks at the end of the string

#Find digits and whitespace
str_view(string5,"\\d") # Find digit
...
df$quotes
str_view(df$quotes,"(Steve|Jobs)")
str_view_all(df$quotes,"[abc]") # Any a, b, or c
str_view_all(df$quotes,"[^abc]") # Anything but a, b, or c
```
...
str_view(df$quotes, "0+") # Returns the largest string of 0's
str_view(df$quotes, "[sn]+") # Returns the larggest string of s or n
str_view_all(df$quotes, "[sn]+") # Find all the matches it can
...
str_view_all(df$quotes, "[sn]{2}") # Finds all combinations of exactly 2
```
...
str_replace(roster$Gender,"(\\bmale\\b|\\bM\\b)", "Male") # explicitly male as the whole word
...
str_replace(roster$Major,"(\\?|)", "NA") # Puts NA's at the start of words as well.
...
s1 = sentences[1]
str_split(s1," ")
str_split(s1,"\\b") # split on boundary
...
NYTIMES_KEY = "FDHOzGhKDUs0ArXViMvSfjMyYe4tbJkm" #Your Key Here … get from NTY API website
...
ArticleToClassify = allNYTSearch[9,] # Check out 4, 9 and 44
ArticleToClassify$response.docs.headline.main
str(ArticleToClassify)
...
wordsToTakeOut = c(stopwords(), "Data", "Science")
